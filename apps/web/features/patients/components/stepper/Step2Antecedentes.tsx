...
export function Step2Antecedentes({ data, onChange, errors = {}, onValidate }: Props) {
  ...
  function textareaStyle(key: string): React.CSSProperties {
    return {
      ...
      outline: 'none', resize: 'vertical', lineHeight: '22px', minHeight: 90,
    }
  }
        ...
        return (
          <div key={key}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              {/* Contador — patrón DS: X / 800 caracteres */}
              <span style={{
                fontSize: '12px', fontWeight: 500,
                color: count >= MAX_CHARS ? 'var(--error)' : count > MAX_CHARS * 0.85 ? 'var(--warning)' : 'var(--ink-400)',
              }}>
                {count} / {MAX_CHARS} caracteres
              </span>
            </div>
            <textarea
              value={value}
              ...
              onFocus={(e) => {
                e.target.style.borderColor = errors[key] ? 'var(--error)' : 'var(--green-700)'
                e.target.style.boxShadow = '0 0 0 3px rgba(45,90,39,0.12)'
              }}
              onBlur={(e) => {
                e.target.style.borderColor = errors[key] ? 'var(--error)' : 'var(--ink-100)'
